...
void setup_ipc_communication_cm0(void)
{
    ...
    //IRQn_Type irqn = user_ipc_pipe_config_cm0.ep0ConfigData.ipcNotifierMuxNumber;
    //NVIC_DisableIRQ(irqn);
    // for some reason the IPC driver does not set priority on CM0+
    //NVIC_SetPriority(irqn, user_ipc_pipe_config_cm0.ep0ConfigData.ipcNotifierPriority); 
    //NVIC_EnableIRQ(irqn);
    //__enable_irq();
}
